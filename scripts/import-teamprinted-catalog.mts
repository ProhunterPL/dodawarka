/**
 * Batch: scrape TeamPrinted Incore Store → allocate EAN → append MojeGS1 → import do Apilo.
 *
 * Usage:
 *   npx tsx scripts/import-teamprinted-catalog.mts --dry-run
 *   npx tsx scripts/import-teamprinted-catalog.mts --limit=1
 *   npx tsx scripts/import-teamprinted-catalog.mts
 */
...
const LIST_URL = "https://teamprinted.pl/s/incore-sports-store/";
...
async function main() {
  ...
  const listRes = await fetch(
    `${BASE}/api/scrape/teamprinted?action=list&url=${encodeURIComponent(LIST_URL)}`,
  );
  ...
  const scrapeRes = await fetch(`${BASE}/api/scrape/teamprinted`, {
    method: "POST",
  ...
  for (const item of scrapeJson.items) {
    ...
    console.log(
      `  OK ${p.sku} | ${p.groupName.slice(0, 70)} | sizes=${p.variants.map((v) => v.size).join(",")} | price=${p.priceWithTax} | imgs=${p.imageUrls.length}`,
    );
    products.push(p);
  }
  ...
  const snapshotPath = path.join(process.cwd(), "data", "teamprinted-import-batch.json");
  writeFileSync(
    snapshotPath,
    JSON.stringify(
      {
  ...
  writeFileSync(
    path.join(process.cwd(), "data", "teamprinted-import-results.json"),
